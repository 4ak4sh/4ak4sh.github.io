...
// Enhanced sticky portfolio behavior for mobile
function initStickyPortfolio() {
  ...
  // Observer for portfolio section - makes it sticky
  ...
  // Observer for services section - continues normal flow after portfolio
  const servicesObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (window.innerWidth <= 768) {
        if (entry.isIntersecting && entry.intersectionRatio > 0.3) {
          currentSection = 'services';
          // After services, allow normal scrolling
          setTimeout(() => {
  ...
  // Touch handling for better swipe control
  let touchStartY = 0;
  let touchEndY = 0;
  ...
  portfolioSection.addEventListener('touchend', (e) => {
    ...
    // If user swipes up significantly while in portfolio
    if (swipeDistance > 80 && currentSection === 'portfolio') {
      servicesSection.scrollIntoView({ 
        behavior: 'smooth',
        block: 'start'
      });
    }
    // If user swipes down significantly while in portfolio
    else if (swipeDistance < -80 && currentSection === 'portfolio') {
